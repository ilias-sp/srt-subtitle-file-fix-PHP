...
function identify_process_line($line, $offset)
{
    ...
    elseif (preg_match('/^(([0-9]{2}):([0-9]{2}):([0-9]{2}),([0-9]{3})([\s]*)(-->)([\s]*)([0-9]{2}):([0-9]{2}):([0-9]{2}),([0-9]{3}))$/i', $line, $matches) === 1)
    {
        ...
        // $new_line = gmdate("H:i:s", $start_time) . ',' . $matches[5] . ' --> ' . gmdate("H:i:s", $end_time) . ',' . $matches[12]; // old way
...
function do_conversion_main($file_name, $offset)
{
    ...
    foreach(preg_split("/((\r?\n)|(\r\n?))/", $srt_contents) as $line)
    {
        // process each $line:
        // 
        $new_line = identify_process_line($line, $offset);
...
if (isset($argv[2]))
{
    ...
    {
        send_console('ERROR', 'No numeric offset provided.');
        send_console('INFO', 'USAGE: <script> <SRT file to work on> <offset in seconds to add/remove>');
        exit;
    }
}
else
{
    send_console('ERROR', 'No offset provided.');
    send_console('INFO', 'USAGE: <script> <SRT file to work on> <offset in seconds to add/remove>');
    exit;
}

do_conversion_main($file_name, $offset);
